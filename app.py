...
from flask import (
    ...
    session, jsonify, Response, abort, flash
)

# Optional Mediapipe (Module 7)
# try:
...
def m1_mjpeg_stream():
    """MJPEG generator for Module 1 video feed."""
    cam = get_camera()
    while True:
        ok, frame = cam.read()
        if not ok:
...
# ============================
# Module 2 – session helpers
# ============================
def m2_rid() -> str:
    """Return (and cache in session) a short per-user runtime id for Module 2."""
...
def m2_session_dir():
    """
    Per-session directory for Module 2.
    Ensures uploads/ and outputs/ exist and returns the root path.
    """
...
def _m2_safe(name: str) -> str:
    """Sanitise filenames for Module 2 uploads."""
    return "".join(
        c for c in name
...
def _m2_ksize_from_sigma(sigma: float) -> int:
    """
    Convert a blur sigma into an odd kernel size (>=3).
    Used for Gaussian blur in Module 2.
    """
...
# ============================
def lbph_create():
    """Create an LBPH face recogniser (requires opencv-contrib-python)."""
...
def ensure_model_trained():
    """Load LBPH model from disk if it exists; otherwise return None."""
...
def module_n(n):
    """
    Basic shell for modules 1–4 that use a simple template.
    (Module 3/4/5/6/7 have their own explicit routes below.)
    """
    if n not in (1, 2, 3, 4):
        abort(404)
...
@login_required
def module_1():
    """Explicit route alias for Module 1."""
    return render_template("modules/module1.html", user=session.get("user"))


# ============================
# Webcam MJPEG stream for login
# ============================

def mjpeg_stream():
    """
    MJPEG generator used on login/register pages
    to show a live face preview.
    """
    cam = get_camera()
    while True:
        ok, frame = cam.read()
        ...
        ok, buf = cv.imencode(".jpg", frame, [cv.IMWRITE_JPEG_QUALITY, 80])
        if not ok:
            continue
...
def video_feed():
    """
    Unprotected stream: needed on login/register before
...
def api_capture():
    ...
    # Keep a simple sequence: img_0001.jpg, img_0002.jpg, ...
    existing = sorted(user_dir.glob("img_*.jpg"))
...
def api_train():
    """
    Train LBPH model on ALL user folders in data/faces/*.
    Each folder name becomes a label (username).
    """
    images, labels = [], []
    label_map = {}
    next_label = 0
    ...
    # Persist threshold (reuse if file already existed)
    META_PATH.write_text(json.dumps({"threshold": LBPH_THRESHOLD}, indent=2))

    return jsonify(
        ok=True,
        msg=f"trained on {len(images)} images, users={len(label_map)}",
    )
...
def api_auth():
    ...
    # Predict (LBPH: lower confidence → better match)
...
# ============================
# Module 1 – Measurement APIs
# ============================

@app.route("/measure/video_feed")
@login_required
def m1_video_feed():
    """Protected Module 1 camera stream (with watermark)."""
    return Response(
        m1_mjpeg_stream(),
        mimetype="multipart/x-mixed-replace; boundary=frame",
    )
...
@login_required
def m1_toggle():
    """Toggle the Module 1 camera flag (for UI)."""
    global M1_CAMERA_ON
...
def m1_capture():
    ...
    out_path = (
        M1_CAL_DIR / f"calib_{ts}.jpg"
        if mode == "calib"
        else M1_CAPT_DIR / f"capture_{ts}.jpg"
    )
...
def m1_calibrate():
    ...
    Runs camera calibration using ALL images in M1_CAL_DIR
    and writes calib.npz with K, dist, rvecs, tvecs, etc.
    """
    try:
    ...
    return jsonify(
        ok=True,
        npz=str(out_npz.relative_to(BASE_DIR)),
...
def m1_measure_perspective():
    ...
    Uses pinhole approximation:
      dX ≈ Z * Δx / fx
      dY ≈ Z * Δy / fy
    """
    try:
        payload = request.get_json(force=True) or {}
        # name is currently unused but kept for future use
        _name = payload["image_name"]
        p1 = payload["p1"]
        p2 = payload["p2"]
    ...
    dX_world = Z * (dx_px / fx) / 2.0
    dY_world = Z * (dy_px / fy) / 2.0
...
def mod2_upload():
    ...
    Form:
      file: uploaded image
    Saves to:
      runtime/module2/sessions/<rid>/uploads/
    Returns:
      path_rel: path relative to BASE_DIR for later use.
    """
...
def mod2_detect():
    ...
    Runs cv.matchTemplate on grayscale images,
    draws a green rectangle on best match, and saves annotated image.
    """
    payload = request.get_json(force=True) or {}
    ...
    # stash ROI box (for UI use) and clear stored blur params
...
def mod2_blur():
    """
    Gaussian blur inside selected ROI.
    ...
    Stores blur params in session for deblurring.
    """
    payload = request.get_json(force=True) or {}
    base_rel = payload.get("base_path_rel")
...
# ============================
# Module 2 – Deblur helpers
# ============================

def _psf2otf(psf, out_shape):
    """
    Convert PSF (spatial domain) to OTF (frequency domain)
    with same shape as out_shape.
    """
    H, W = out_shape
    kh, kw = psf.shape
...
def _wiener_deconv(gray_roi, ksize=9, sigma=2.0, K=1e-3):
    """
    Simple Wiener deconvolution on a single-channel ROI.
    """
...
def mod2_deblur():
    """
    Deblur previously blurred region using Wiener deconvolution.
    ...
    Uses stored blur params from session["m2_blur_params"].
    """
    payload = request.get_json(force=True) or {}
    ...
    # deconvolve only the luminance (Y) channel
...
def mod2_file(rel):
    """
    Serve images only from this user's Module 2 session folder.
    """
...
# ============================
# Module 3 – Offline Image Processing Suite
# ============================

# ----- Dashboard & Task Dispatcher -----
...
def module3_task(num):
    """
    Dispatcher so the dashboard can link to:
      url_for('module3_task', num=1..5)
    """
    if num == 1:
        ...
        return redirect(url_for("module3_task5"))
    abort(404)


# ----- Shared helpers for Module 3 -----
...
def module3_task2():
    ...
    Corner points (green) via Harris response.
    """
    results = []
...
def module3_task3():
    """
    blur → Canny → morphology → largest contour → draw polygon boundary (blue).
    """
    results = []
...
def module3_task4():
    """
    Detect ArUco (4X4_1000 → 250 → 100 → 50). Fallback to QR if none.
    ...
    if request.method == "POST":
        ...
        for img_path in images_to_process:
            ...
            if len(all_pts) >= 3:
                ...
                cv.polylines(overlay, [hull], True, (255, 0, 0), 2)  # blue outline
                color_fill = np.zeros_like(hull_vis)
                color_fill[:] = (255, 200, 0)
...
@login_required
def module3_task5():
    """Simple redirect to external SAM2 demo."""
    return redirect("https://segment-anything.com/demo#")


# ============================
# Module 4 – Panorama & SIFT-lite Matching
# ============================

# ----- File serving for Module 4 -----

@app.get("/mod4/file/<path:rel>")
@login_required
def mod4_file(rel):
    """
    Serve images from this user's Module 4 session folder only.
    """
    p = (BASE_DIR / rel).resolve()
    sd = (M4_SESSIONS / m4_rid()).resolve()
    if not str(p).startswith(str(sd)) or not p.exists():
        abort(403)
    # Adjust mimetype if you later serve PNG/WEBP etc.
    return Response(open(p, "rb").read(), mimetype="image/jpeg")
...
def module_4():
    """
    Module 4 index (links to Task 1: Panorama, Task 2: SIFT-lite).
    """
    return render_template("modules/module4/index.html", user=session.get("user"))


# ----- Common helpers for Module 4 -----
...
def m4_uniq(stem: str, ext: str) -> str:
    """
    Unique filename with short random suffix.
    """
...
def m4_imread_color_path(path: Path):
    """
    Robust imread using np.fromfile → cv.imdecode (Windows-safe).
    """
...
# ============================
# Module 4 – Task 1: Panorama Stitching
# ============================
...
def module_4_task1():
    """
    Upload multiple overlapping images and build a stitched panorama.
    """
    notes = []
    result_rel = None
...
# ============================
# Module 4 – Task 2: SIFT-lite vs OpenCV SIFT
# ============================
...
def module_4_task2():
    """
    Compare custom SIFT-lite descriptors vs OpenCV SIFT on two images.
    """
    notes, metrics = [], {}
...
# ==========================
# Module 7 – pose/hand stream state
# ==========================
M7_T3_FRAME_IDX = 0
...
def module7_task1():
    ...
    if request.method == "POST":
        ...
        if (
            not left_file
            or left_file.filename == ""
            or not right_file
            or right_file.filename == ""
        ):
...
def module7_task1_measure():
    ...
    if not np.any(valid):
        return jsonify(
            ok=False,
            error="Disparities too small; make sure you click matching points in left and right images.",
        )
        ...
        if HAVE_MEDIAPIPE:
            ...
            if hands_res.multi_hand_landmarks:
                for lm, handed in zip(
                    ...
                    label = (
                        handed.classification[0].label
                        if handed.classification
                        else "unknown"
                    )
                    mp_drawing.draw_landmarks(
                        frame,
